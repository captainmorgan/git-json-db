...
//               {"tag":"3", "dog_name":"Ruby", "dog_id": "4", "customer_last_name":"Morgan", "status":"In", "trainer":"Trainer", "location":"Kettner Arts", "sublocation":"Dog Bowl", "time_check_in":"2014-05-29 15:29:00", "time_check_out":""}
	...
	public function getData() {
    	...
    	$this->ResponseBody = $data->dBQuery(trim("{\"table\": \"daycare\", \"join\": \"dog\", \"fields\": [\"dog.id\", \"tag\", \"dog.dog_name\", \"dog.id\", \"customer_last_name\", \"dog.owner_id\", \"location\", \"sublocation\", \"trainer\", \"time_check_in\", \"updated\", \"dog.breed\", \"dog.dog_notes\", \"dog.behaviors\", \"Status\"], \"where\": {\"Status\":\"In\"}, \"on\":{\"daycare\":\"dog_id\", \"dog\":\"id\"}}"));
	...
	// Response Body: 0, if the customer doesn't have any dogs
	// Response Body: -1, if the email address was not valid
	public function getDogsByEmail() {
		
		$this->connectDB();
    	$data = new GetRecordInnerJoin($this);

    		$this->ResponseBody = $data->dBQuery(trim("{\"table\": \"customer\", \"join\": \"dog\", \"fields\": [\"first_name\", \"last_name\", \"email\", \"dog.id\", \"dog_name\", \"dog_notes\", \"breed\", \"owner_id\"], \"where\": {\"email\":\"" . $this->payload . "\"}, \"on\":{\"customer\":\"id\", \"dog\":\"owner_id\"}}"));
		
		// We want to differentiate if the Customer whose email was provide either doesn't exist, or just doesn't have any dogs
		// A null response was returned from the inner join (meaning something isn't right...let's see which)
		if ($this->ResponseBody == '[]') {
			//PC::debug("null" . $this->ResponseBody);
			...
			// Email is not valid
			if ($this->ResponseBody == '[]') {
				//PC::debug("Email provided does not belong to a registered customer. " . $this->ResponseBody);
				$this->disconnectDB();
				echo "-1";
			}
			// Email is valid, just no current dogs
			else if ($this->ResponseBody != '[]') {
				//PC::debug("This customer does not have any registered dogs. " . $this->ResponseBody);
				$this->disconnectDB();
				echo "0";				
			}
		...
		// We have a valid result list from the inner join.  The email belongs to a customer who has dogs.
		else {
			//PC::debug("The email belongs to a customer who has dogs. " . $this->ResponseBody);
			echo $this->ResponseBody;
			$this->disconnectDB();		
		}
	...
	public function getDogsByOther() {
		...
		if ($this->ResponseBody == '[]') {
			//echo " marketing: ";
			$this->ResponseBody = $data->dBQuery(trim("{\"table\": \"customer\", \"join\": \"dog\", \"fields\": [\"first_name\", \"last_name\", \"email\", \"dog.id\", \"dog_name\", \"dog_notes\", \"breed\", \"owner_id\"], \"where\": {\"marketing_email\":\"" . $this->payload . "\"}, \"on\":{\"customer\":\"id\", \"dog\":\"owner_id\"}}"));
		}
		
		if ($this->ResponseBody == '[]') {
			//echo " phone: ";
			$this->ResponseBody = $data->dBQuery(trim("{\"table\": \"customer\", \"join\": \"dog\", \"fields\": [\"first_name\", \"last_name\", \"email\", \"dog.id\", \"dog_name\", \"dog_notes\", \"breed\", \"owner_id\"], \"where\": {\"phone\":\"" . $this->payload . "\"}, \"on\":{\"customer\":\"id\", \"dog\":\"owner_id\"}}"));
		}

		if ($this->ResponseBody == '[]') {
			//echo " last name: ";
			$this->ResponseBody = $data->dBQuery(trim("{\"table\": \"customer\", \"join\": \"dog\", \"fields\": [\"first_name\", \"last_name\", \"email\", \"dog.id\", \"dog_name\", \"dog_notes\", \"breed\", \"owner_id\"], \"where\": {\"last_name\":\"" . $this->payload . "\"}, \"on\":{\"customer\":\"id\", \"dog\":\"owner_id\"}}"));
		}

		if ($this->ResponseBody == '[]') {
			//echo " last name: ";
			$this->ResponseBody = $data->dBQuery(trim("{\"table\": \"customer\", \"join\": \"dog\", \"fields\": [\"first_name\", \"last_name\", \"email\", \"dog.id\", \"dog_name\", \"dog_notes\", \"breed\", \"owner_id\"], \"where\": {\"dog_name\":\"" . $this->payload . "\"}, \"on\":{\"customer\":\"id\", \"dog\":\"owner_id\"}}"));
		}	
...
// New methods follow
	...
	// Method to return the number of times a customers dogs have stayed at daycare, given
	// a Customer email address.  Return "-1" is a number wasn't provided.
	// Usage Example:
	// api.php?method=getCustomerVisitCount&payload=108
	// Response Body: 1
	public function getCustomerVisitCount() {
		$this->connectDB();
		$cc = new GetRecordCount($this);
		
		if (is_numeric($this->Request['payload'])) {
			$this->ResponseBody = $cc->dBCount(trim("{\"table\": \"daycare\", \"fields\": [\"*\"], \"where\":{\"owner_id\":\"" . $this->payload . "\"}}"));
			echo $cc->count;
		}
		// Return a "-1" is a number wasn't provided.
		else {
			echo "-1";
		}
	...
	} // End function getCustomerVisitCount
