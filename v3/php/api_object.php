...
//*********************************
    ...
    // Request Body: api.php?method=descSchema&payload=employee
    ...
    public function getRecord() {
    	...
    	//DBConfig::write('db.password', '11sumrall11');
    	//DBConfig::write('db.password', self::getPassword('captain_read')); // works with the internal captain password functin
    	//DBConfig::write('db.password', $pw); // works
    	$this->connectDB();
	...
	// Experimental
	// getPassword is a protected function that is designed to return the captain passwords.
	// The "captains" are MySQL-users, not application users and the concept is that each CRUD method
	// Should only talk to the database using the appropriate captain.  This enforces least-privledge and adds a deep layer of security
	// This problem is that these passwords need to be hard-coded in the PHP, which doesn't scale well.
	// There is a table called, "sys_captain" which contains fields for the captain name and the associated password.
	//
	// getPassword should never be called from the POST API -- it should only be called internally using:
	//		self::getPassword('captain_read')   or
	//		parent::getPassword('captain_read') from an extended class
	//
	// TODO:  Eventually this method should handle password decryption, since we really don't want to store the captain passwords in plain text.
	protected function getPassword($captain) {
	// {"table": "sys_captain", "fields": ["password"], "where":{"username":"captain_write"}}
	
    	DBConfig::write('db.user', 'captain_read');
    	DBConfig::write('db.password', '11sumrall11');
    	$this->connectDB();
    	$cc = new GetRecord($this);
		$this->ResponseBody = $cc->dBQuery('{"table": "sys_captain", "fields": ["password"], "where":{"username":"' . $captain . '"}}');

		$pw = json_decode($this->ResponseBody, true);
		//echo $pw[0]['password'];
		// We want it silent
	...
	public function callGetPassword() {
	
		self::getPassword('captain_read');
	
	} // End function callGetPassword
	
	public function callPwTest() {
	
		echo "callPwTest is working";
