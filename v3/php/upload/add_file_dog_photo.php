...
$sql_host       = DBConfig::read('db.host');       
        ...
        $size = intval($_FILES['uploaded_file']['size']);
 		$id = intval($_POST['dog_lookup_id']);
 		$description = ($_POST['photo_caption']);
 		$table = "dog";
 
        // Create the SQL query
        $query = "
            INSERT INTO `photo` (
                `name`, `mime`, `size`, `data`, `description`, `linked_table`, `linked_id`, `created`
            )
            VALUES (
                '{$name}', '{$mime}', {$size}, '{$data}', '{$description}', '{$table}', {$id}, NOW()
            )";
