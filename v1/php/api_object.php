...
//*********************************
    ...
    // Experimental
    public function descSchema() {
    	$this->connectDB();
    	$dsc = new DescSchema($this);
    	//$dsc->setRequestBody($this->RequestBody);
    	//$dsc->queryDB("customer");
    	$dsc->setTable($this->RequestBody);
    	echo $dsc->queryDB();
    	$this->disconnectDB();
    }
    ...
    public function createCustomer() {
       ...
       // Response: 232, where that number is the newly created customer's ID
        ...
        $cust->setRequestBody($this->RequestBody);
       	$cust->setSchemaJSON($this->schemaLabel_customer); // Since thsis is create*Customer*, this is hard-linked
