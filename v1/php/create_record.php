...
/*
	...
	--
		...
		Data JSON Example:
		{"salutation":"Mr","first_name":"Christopher","last_name":"Morgan","phone":"8083669175","email":"sw2342343@gmdddail.com","marketing_email":"","address_street":"2146 Erie Street","address2":"","city":"San Diego","state":"CA","zip":"92110","country":"United States","company":"","work_phone":"8083669175","work_street1":"","work_street2":"","work_city":"San Diego","work_state":"CA","work_zip":"","work_country":"United States","email_pref":"1","referral":"Something-else","dob":"1960-01-01"}
...
public function ssjp($s)
{
echo $s;
$this->setSchemaJSON($s);
}
public function sdjp($s)
{
echo $s;
$this->setDataJSON($s);
}
public function dpi($s)
{
echo $s;
$this->dBInsert($s);
}
...
*/			
		try {
			...
			foreach ($this->dataArr as $key => $value) {
				$name = ':'.$key;
				$sql->bindValue($name, trim($value), PDO::PARAM_STR);
			}		
			$sql->execute();
			//echo $conn->lastInsertId();
			$this->createdID = (int)$this->parent_object->conn->lastInsertId();
